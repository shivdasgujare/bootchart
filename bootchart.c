...
#define LOG_STARTFILE   LOG_ROOT"/bootchart-start"
#define LOG_STOPFILE    LOG_ROOT"/bootchart-stop"
...
int main(int argc, char **argv)
{
	...
	if (bootchart_count) {
		printf("bootcharting started (period=%d ms) (timeout mode)\n", bootchart_count*BOOTCHART_POLLING_MS);
	} else if (!bootchart_count) {
		printf("starting bootchart in manual mode.. plz, echo 1 to bootchart-stop file\n");
	} else {
		printf("bootcharting init failure\n");
	}

	for (;;) {
		usleep(1000*BOOTCHART_POLLING_MS); //msec steps for taking logs
		if (EXIT_ON_TIMEOUT) {
